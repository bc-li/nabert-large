...
class NumericallyAugmentedBERTPlus(Model):
    ...
    def __init__(self, 
        ...
        # TODO: CHANGE BERT -> ROBERTA

        self.roberta = RobertaModel.from_pretrained("roberta-base")
        self.tokenizer = RobertaTokenizer.from_pretrained("roberta-base")
        roberta_dim = self.roberta.pooler.dense.out_features
        ...
        self._passage_weights_predictor = torch.nn.Linear(roberta_dim, 1)
        self._question_weights_predictor = torch.nn.Linear(roberta_dim, 1)
        self._number_weights_predictor = torch.nn.Linear(roberta_dim, 1)
        self._arithmetic_weights_predictor = torch.nn.Linear(roberta_dim, 1)

        if len(self.answering_abilities) > 1:
            self._answer_ability_predictor = \
                self.ff(2 * roberta_dim, roberta_dim, len(self.answering_abilities))

        if "passage_span_extraction" in self.answering_abilities:
            self._passage_span_extraction_index = self.answering_abilities.index("passage_span_extraction")
            self._passage_span_start_predictor = torch.nn.Linear(roberta_dim, 1)
            self._passage_span_end_predictor = torch.nn.Linear(roberta_dim, 1)
        ...
        if "question_span_extraction" in self.answering_abilities:
            ...
            self._question_span_start_predictor = \
                self.ff(2 * roberta_dim, roberta_dim, 1)
        ...
        if "arithmetic" in self.answering_abilities:
            ...
            self.special_embedding = torch.nn.Embedding(self.num_special_numbers, roberta_dim)
            if self.arithmetic == "base":
                self._number_sign_predictor = \
                    self.ff(2 * roberta_dim, roberta_dim, 3)
            else:
                self.init_arithmetic(roberta_dim, roberta_dim, roberta_dim, layers=2, dropout=dropout_prob)

        if "counting" in self.answering_abilities:
            self._counting_index = self.answering_abilities.index("counting")
            self._count_number_predictor = \
                self.ff(roberta_dim, roberta_dim, max_count + 1) 
    ...
    def init_arithmetic(self, roberta_dim, input_dim=300, rnn_hidden_dim=500, layers=1, dropout=0.3):
        ...
        self.Wst = torch.nn.Linear(rnn_hidden_dim, roberta_dim)
        self.Wo = torch.nn.Linear(roberta_dim, roberta_dim)
        self.Wc = torch.nn.Linear(roberta_dim, roberta_dim)
        
        self.Wcon = torch.nn.Linear(roberta_dim, roberta_dim)
        ...
        self.num_ops = 5
        self.arithmetic_K = 5
        self.op_embeddings = torch.nn.Embedding(self.num_ops + 1,roberta_dim)
        self.ops = ["END", "+", "-", "*", "/", "100"]
        self.END = 0
    ...
    def forward(self,  # type: ignore
        ...
        roberta_out,_ = self.roberta(question_passage_tokens,pad_mask,output_hidden_states =False)
        ...
        # f.write(type(question_end))
        question_out = roberta_out[:,:question_end]
...
#             arithmetic_summary = self.summary_vector(question_out, question_mask, "arithmetic")
    ...
    def _adv_arithmetic_module(self, summary_vector, maxlen, options, options_mask, roberta_out, bert_mask):
        ...
        alpha = torch.bmm(out, roberta_out.transpose(1,2))
        alpha = util.masked_softmax(alpha, bert_mask)
        
        # context : (batch, explen, bert_dim)
        context = util.weighted_sum(roberta_out, alpha)
