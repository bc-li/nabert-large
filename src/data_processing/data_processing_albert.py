...
import pickle
import os
# BERT_PATH = '/data/bochengli/drop-bert/drop_bert_old/bert-base-uncased'
...
@Tokenizer.register("albert-drop")
class BertDropTokenizer(PretrainedTransformerTokenizer):
    def __init__(self, pretrained_model: str):
        super().__init__(
    ...
    @overrides
    def tokenize(self, text: str) -> List[Token]:
        return [Token(token) for token in self.tokenize(text)]
@TokenIndexer.register("albert-drop")
class AlbertDropTokenIndexer(PretrainedTransformerIndexer):
    def __init__(self,
